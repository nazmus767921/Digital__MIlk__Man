...
const initialState = {
	...
	isEditing: { status: false, id: "", index: 0 },
	...
	switch (action.type) {
		...
		case "SUBMIT_INPUT":
			if (!state.name) {
				return (state = {
					...state,
					alert: { show: true, msg: "Empty Input", type: "safe" },
				});
			} else if (state.name && state.isEditing.status) {
				// setList(
				// 	list.map((item) => {
				...
				const newList = { id: crypto.randomUUID(), name: state.name, logDate };
				const UpdatedList = [...state.list, newList];

				return (state = { ...state, name: "", list: UpdatedList });
...
const LetListApp = () => {
	...
	//! Event Handlers
	const handleInput = (e) => {
		dispatch({ type: "TAKE_INPUT", payload: e.target.value });
	};
